...
| resultCode | string \| null | N | null | `SUCCESS`·`USER_DENIED`·`DECRYPT_FAILED`·`DELIVERY_FAILED` 중 하나(`BIZ-001-01`). null = 결과 미확정 | - | 결과 구분. 한 번 확정되면 바뀌지 않는다(`BIZ-001-04`) |
...
| SVC-003 | 동의·승인 | 도메인 | PROC-103 · PROC-301 | 거부 시 `USER_DENIED` 확정 |
...
- **결과 구분은 문자열 상수 4개를 한 곳에 정의하고 화면·API·집계가 그 정의를 가져다 쓴다.** 모델 계층에서 별칭·대소문자 변형을 만들지 않는다(`BIZ-001-03`).
