...
|--------|-----------|----------------|------|--------|-----------|----|------|
| request_key | UNIQUEIDENTIFIER | - | NOT NULL | - | - | PK | 요청 키값(허브 발급 불투명 UUID v4, DATA-002). 회원 키와 무관 |
| config_id | UNIQUEIDENTIFIER | - | NOT NULL | - | - | FK | 연동 구성 참조(ENT-001.id, 개인정보 아님) |
| is_success | BIT | - | NOT NULL | - | - | - | 처리 성공 여부(1=전달 성공, 0=실패·거부 미전달) |
| is_result_confirmed | BIT | - | NOT NULL | 0 | - | - | 결과 확인 여부(서비스 A 조회 성공 시 1) |
| processed_at | DATETIME2 | 3 | NOT NULL | - | - | - | 처리 일시(연동 실행 결과 확정 시각) |
| result_confirmed_at | DATETIME2 | 3 | NULL | NULL | - | - | 결과 확인 일시(미확인 시 NULL) |
| created_at | DATETIME2 | 3 | NOT NULL | SYSUTCDATETIME() | - | - | 레코드 생성 일시(영속화 감사) |

> **무결성 CHECK**: `(is_result_confirmed = 1 AND result_confirmed_at IS NOT NULL) OR (is_result_confirmed = 0 AND result_confirmed_at IS NULL)` — 결과 확인 여부와 확인 일시의 정합을 DB 레벨에서 강제(배치 삭제 기준 정확성 보장).
...
|----------|-----------|------|-----------------|----------------------|
| PK_PROCESS_STATUS | request_key | PK·UNIQUE(NONCLUSTERED 권장) | 높음 | PROC-301 상태 조회·갱신(WHERE request_key=?), PROC-401 INSERT 중복 방지 |
| IX_STATUS_RETENTION_PENDING | processed_at | BTREE(필터: WHERE is_result_confirmed = 0) | 중간 | PROC-402 미완료 삭제 대상 선정(WHERE is_result_confirmed=0 AND processed_at < 기준, DATA-004-02) |
| IX_STATUS_RETENTION_CONFIRMED | result_confirmed_at | BTREE(필터: WHERE is_result_confirmed = 1) | 중간 | PROC-402 완료 삭제 대상 선정(WHERE is_result_confirmed=1 AND result_confirmed_at < 기준, DATA-004-01) |

> 두 필터 인덱스는 배치의 완료/미완료 두 갈래(BR-401) 범위 삭제를 각각 지원한다. config_id 는 조건절·정렬·조인에 쓰는 PROC 가 없어 인덱스를 신설하지 않는다(사용 PROC 0건 금지 규칙).
...
- **수정 조건**: PROC-301(처리상태 확인 API) · "조회 성공 후 상태 갱신"에서 최초 조회 성공 시 is_result_confirmed=1·result_confirmed_at 갱신(BR-301, 멱등 — 재조회 시 미갱신).
...
- request_key 는 랜덤 UUID v4 이므로 PK 를 NONCLUSTERED 로 두고, 물리 클러스터링 키는 processed_at 기반(예: processed_at, request_key)으로 잡아 삽입 단편화를 줄이고 배치 범위 삭제 지역성을 높이는 것을 권장한다(build 확정).
