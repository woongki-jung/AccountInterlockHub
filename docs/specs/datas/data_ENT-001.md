...
|--------|-----------|----------------|------|--------|-----------|----|------|
| id | UNIQUEIDENTIFIER | - | NOT NULL | NEWSEQUENTIALID() | - | PK | 구성 고유 식별자(내부) |
| config_code | NVARCHAR | 64 | NOT NULL | - | LEN(config_code) > 0 | UK(필터) | 구성 식별자(업무 코드, BIZ-001-03 고유) |
| config_name | NVARCHAR | 100 | NOT NULL | - | LEN(config_name) > 0 | - | 구성명(표시용) |
| service_a_entry_url | NVARCHAR | 2048 | NOT NULL | - | LIKE 'http://%' OR LIKE 'https://%' | - | 서비스 A 호출(진입) 주소(BIZ-001-02) |
| service_b_delivery_url | NVARCHAR | 2048 | NOT NULL | - | LIKE 'http://%' OR LIKE 'https://%' | - | 서비스 B 전달 주소(BIZ-001-02·BIZ-003-02) |
| service_b_http_method | NVARCHAR | 10 | NOT NULL | 'POST' | IN ('GET','POST','PUT','PATCH') | - | 서비스 B 전달 방식(HTTP 메서드) |
| is_active | BIT | - | NOT NULL | 1 | - | - | 활성 여부(성과 지표 "활성 연동 구성 수" 근거) |
| created_at | DATETIME2 | 3 | NOT NULL | SYSUTCDATETIME() | - | - | 생성 일시(감사) |
| created_by | NVARCHAR | 64 | NOT NULL | - | - | - | 생성자 관리자 계정(ENT-005.username) |
| updated_at | DATETIME2 | 3 | NULL | NULL | - | - | 최종 수정 일시(감사) |
| updated_by | NVARCHAR | 64 | NULL | NULL | - | - | 최종 수정자 관리자 계정 |
| deleted_at | DATETIME2 | 3 | NULL | NULL | - | - | 소프트 삭제 시각(NULL=유효) |
...
|----------|-----------|------|-----------------|----------------------|
| PK_INTERLOCK_CONFIG | id | PK(NONCLUSTERED 권장) | 높음 | PROC-101 INSERT/UPDATE, FK 조인 대상 |
| UQ_CONFIG_CODE | config_code | UNIQUE(필터: WHERE deleted_at IS NULL) | 높음 | PROC-101 고유성 사전 조회(WHERE config_code=?), PROC-102 상세, PROC-201 활성 구성 조회 |
| IX_CONFIG_LIST | is_active, created_at DESC | BTREE(필터: WHERE deleted_at IS NULL) | 낮음~중간 | PROC-102 목록 조회·필터·정렬(WHERE is_active=? ORDER BY created_at DESC) |

> UQ_CONFIG_CODE 는 필터 유니크(deleted_at IS NULL)로, 소프트 삭제된 구성 코드의 재사용을 허용하되 유효 구성 간 고유성을 보장한다(BIZ-001-03·EXC-BIZ-02).
...
- id 는 NONCLUSTERED PK + NEWSEQUENTIALID() 로 두어 GUID 삽입 단편화를 줄인다(물리 클러스터링 키는 build 확정). config_code UNIQUE 는 필터 인덱스로 소프트 삭제와 양립시킨다.
