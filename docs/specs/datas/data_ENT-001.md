...
- **데이터 목적**: 연동 관리자가 등록하는 서비스 A↔서비스 B 연동 1건의 마스터 설정을 영속화한다. 사용자 동의 화면·연동 실행·서비스 B 전달·처리 상태 저장이 모두 본 엔터티를 전제·참조한다. 사용자 키값 파라미터 지정(user_key_param_id)이 연동이력 기록·완료 확인·완료 콜백의 대상 여부를 결정한다(BIZ-004).
...
| user_key_param_id | uuid | - | NULL | NULL | - | FK | 사용자 키값 파라미터 지정(ENT-003.id 참조, 선택 입력 — BIZ-001-07). NULL=미지정(연동이력·완료 확인·완료 콜백 대상 밖, BIZ-004-05) |
...
> user_key_param_id 는 전달 파라미터 정의(ENT-003)에 대한 **참조 저장**이다(값 복제 금지 — BIZ-001 구현 가이드). 단일 컬럼이므로 "구성당 최대 1개"(BIZ-001-07)가 구조적으로 보장되고, 지정 여부 판정(BIZ-004-05, PROC-302·303 사전 검증)이 구성 행 단독 조회로 끝난다.
...
| ENT-003 전달 파라미터(지정 참조) | N:0..1 | user_key_param_id | RESTRICT | NO ACTION | 사용자 키값 파라미터 지정. 지정 유지 상태의 해당 파라미터 행 삭제를 DB 가 차단(BIZ-001-07 응용 검증의 안전망) |
