...
| MDL | 후보 대비 | 확정/변경 내용 |
|-----|-----------|----------------|
| **MDL-101** | **변경** | 중첩 Parameter 에 **isUserKey(사용자 키값 파라미터 지정, 필수)** 추가 — true 구성당 정확히 1개(exactly-one·0개/2개↑ 422, BIZ-001-07), 영속화는 ENT-001.user_key_param_id 참조 저장. MDL-102~104 는 유지 |
...
| **MDL-204** | **변경** | 전달 페이로드. EXT·무저장·무변형 유지 + **configCode·requestKey 동봉**(완료 콜백 회신 계약 — SVC-005 F-003, 전 구성 공통·계약 단일화) |
...
- 깨진 참조 0건 — 모든 MDL 이 ENT 매핑 또는 명시적 무매핑(전송 전용·앱 세션·조건 참조)으로 정의됨. 사용 PROC 는 프로세스 도메인에 실재한다(신규 PROC-302·303·403 포함).
