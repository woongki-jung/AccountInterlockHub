# 발송처 접근 주소 구성 조회·목록·활성/삭제 테스트 케이스 정의 (ADM-02)

## 개요

- **검증 목적**: 발송처 접근 주소 구성의 목록(MDL-102)·상세(MDL-101) read-only 조회, 활성/비활성 전환(PROC-105), 소프트 삭제(PROC-106)가 정확히 동작하고, 삭제된 구성 제외·config_code(접근 주소 고유 ID) 재사용·감사 기록이 정책대로 처리되는지 검증한다. `#214` 로 상세 표시에서 전달 파라미터가 제거되어 구성 항목이 고유 ID·수신처 B 주소·method·동의 항목으로 한정됐다.
- **관련 PRD 요구사항**: [`../../../prd/PRD.md`](../../../prd/PRD.md) §수행 범위·§성과 지표 "활성 연동 구성(발송처 접근 주소) 수". 검증 대상 PROC-102·105·106(SVC-002).
...
| 단계 | 실행 | 입력 | 기대 결과 | 매핑 PROC |
|--|--|--|--|--|
| 1 | GET /configs/:id | 유효 id | 200 MDL-101(수신처 B 주소·method·동의항목 display_order 정렬) | B3 |

- **화면 검증**: SCR-004 Card 렌더. 수신처 B URL 마스킹 없음(EXC-SEC-05), 회원 키·암호값·전달 파라미터·처리 상태 필드 부재.
...
### ADM-02_005 조회 조건 주입·스크립트 문자 정상 조회(바인딩 단독 방어)
- **유형/우선순위/자동화**: Positive(보안) · 높음 · 자동 | **PROC/분기**: PROC-102 / SEC-004-01(b)·SEC-004-02
- **정책 근거**: SEC-004-01 (b) 파라미터 바인딩 단독 방어 확정(2026-07-09 담당자 결정, qa #54) — keyword 허용 문자·주입 패턴 재검증 미요구.

| 단계 | 실행 | 입력 | 기대 결과 | 매핑 PROC |
|--|--|--|--|--|
| 1 | GET /configs?keyword= | 주입 패턴·스크립트 문자(길이 100자 이내) | 200 `{data:[...]}`(거부 없음, 정상 조회) | B1·B2 (SEC-004-02 바인딩) |

- **데이터 검증**: keyword 가 LIKE 파라미터 바인딩으로 안전 대입(`DROP TABLE` 미실행·데이터 무손실). 길이 초과(101자)는 여전히 400 EX-SEC-004(ADM-02_014), 활성 필터 비boolean 형식 위반도 400(ADM-02_009).
...
| 2 | 이후 발송처 링크 진입(PROC-201) | 해당 config_code(접근 주소 고유 ID) | 유효 활성 구성 아님 → 400 EX-SEC-004(is_active=true 조건 배제) | (연계) |
...
|--|--|--|--|
| 데이터 노출 | SCR-002·004 | 설정 데이터만(MDL-101/102) | 회원 키·암호값·전달 파라미터·처리 상태 필드 부재 |
| 마스킹 예외 | SCR-004 | 수신처 B URL 표시 | 마스킹 없음(설정 데이터, EXC-SEC-05) |
