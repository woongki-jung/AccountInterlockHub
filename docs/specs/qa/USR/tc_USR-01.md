...
### USR-01_007 진입 파라미터 주입 문자 안전 처리·전송 형식 검증 유지
- **유형/우선순위/자동화**: Positive/Negative(보안) · 높음 · 자동 | **PROC/분기**: PROC-201 / SEC-004-01(b)·SEC-004-02·EXC-SEC-02
- **정책 근거**: SEC-004-01 (b) 파라미터 바인딩 단독 방어 확정(2026-07-09 담당자 결정, qa #54) — 허용 문자·주입 패턴 재검증 미요구. 신뢰 위임(SEC-002)의 회원 키·파라미터 값에 담긴 주입 문자는 바인딩으로 무실행 저장(200)한다. 단 전송 계층 형식·크기 검증(EXC-SEC-02)은 유지한다.

| 단계 | 실행 | 입력 | 기대 결과 | 매핑 PROC |
|--|--|--|--|--|
| 1 | POST /interlock/entry | 유효 활성 configCode + 파라미터 값에 SQL/스크립트 주입 문자(형식·길이 정상) | 200 `{requestKey}`(거부 없음), 값 원문 무변형 바인딩 저장(무실행) | B1a(SEC-004-02) |
| 2 | POST /interlock/entry | 파라미터 비문자열(형식 위반) 또는 지정 키값 512자 초과(크기) | 400 EX-SEC-004(전송 형식·크기 검증 유지) | B1a(FN-005/FN-016) |

- **비고**: 진입 엔드포인트는 POST(회원 키 URL 노출 방지, 확정 ①). 유효하지 않은 구성 참조(비활성·미존재 configCode)는 400 EX-SEC-004 스코프 거부로 USR-01_003 이 별도 검증한다.
