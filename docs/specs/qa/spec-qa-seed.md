...
| 시각 | 부분 모킹 | **기본은 시드 조정**이다 — 보관 경계·일자 경계 TC 는 **시드 데이터의 일시를 조정**해 만든다(명령 진입점이 기준 일시 인자를 받지 않으므로 임계값을 주입할 수 없다). **예외는 `CHK-CLOCK` 실행 시각 제어가 필요한 3건**이며, 이유가 둘로 갈린다 — ⓐ **기산 방식 자체를 보는 2건**(`BAT-02_016` 월말 clamp · `BAT-02_017` 달력 프레임): 임계값이 `baseAt` 의 **벽시계 일자·시각대**에서 산출되므로(`DATA-002-06` ①③) 결함이 *실행 일자가 월말인 회차* · *KST 일자와 UTC 일자가 갈리는 시각대의 회차*에만 드러나고 시드를 아무리 옮겨도 그 회차에 도달할 수 없다. ⓑ **경계 부등호를 보는 1건**(`BAT-02_007`): `DATA-002-06` ④ 로 기준 일시가 `baseAt` 의 **시·분·초를 그대로** 가지므로, `baseAt` 을 실행 전에 알지 못하면 **"정확히 기준 일시인 행"을 만들 수 없다**(시드는 실행 **전에** 들어간다). `baseAt` 이 소수부 없이 **초까지**로 확정돼 있어([`../functions/spec-functions-api-server.md`](../functions/spec-functions-api-server.md) §출력 스키마 — 버림) 고정한 초 값이 곧 `baseAt` 이고 임계값을 실행 전에 손계산할 수 있다. 미구축 시 세 TC 는 🟠 Block |
...
| `SEED-TRK-BOUNDARY` 경계 3종 | 3 | SQL | 각 기준 일시의 1초 전 / 정확히 / 1초 후에 해당하는 행. **임계값은 `CHK-CLOCK` 으로 고정한 `baseAt`(초 단위·소수부 없음)에서 실행 전에 손계산한다** — `DATA-002-06` ④ 로 기준 일시가 `baseAt` 의 시·분·초를 그대로 가지므로 "정확히" 행은 그 값을 알아야 구성된다. 실행 후 요약의 `baseAt` 이 고정값과 문자열까지 같은지 대조해 시드가 옳은 임계값 기준이었음을 확인한다(다르면 🟠 Block·재실행) | `BAT-02_007` |
...
| `SEED-CSP-BOUNDARY` 경계 3종 | 3 | `tbl_consent_proof` | SQL | `consented_at` 기준 일시의 1초 전 / 정확히 / 1초 후. 임계값 산출·확인 방법은 `SEED-TRK-BOUNDARY` 와 같다(`CHK-CLOCK` 고정 `baseAt` 에서 실행 전 손계산 · 실행 후 문자열 대조) | `BAT-02_007` |
...
| `CHK-CLOCK` 실행 시각 제어 절차 | 절차 | 배치·계수의 **실행 시각을 `Asia/Seoul` 벽시계로 지정**해 재현 가능하게 만드는 절차. 보관 배치는 **인자를 두지 않아 기준 일시를 주입할 수단이 없으므로**(`DATA-002-04`) 실행 시각 자체를 고정하는 것이 유일한 수단이다. **요구 수준은 "그 실행이 취할 `baseAt` 을 실행 전에 초 단위로 예측할 수 있을 것"** — 벽시계를 고정하거나 지정한 초에 실행을 시작시킨다. `baseAt` 은 소수부 없이 초까지이고 초 미만을 **버림**하므로([`../functions/spec-functions-api-server.md`](../functions/spec-functions-api-server.md) §출력 스키마) 지정한 초 값이 곧 `baseAt` 이고, 이 예측 가능성 위에서만 `BAT-02_007` 의 "정확히 기준 일시인 행"과 세 TC 의 손계산 임계값이 성립한다. 확인 방법: 지정한 벽시계로 배치를 1회 실행하고 요약의 `baseAt` 이 **그 값과 `+09:00` 오프셋 표기까지 문자열 동등**인지 대조한다(어긋나면 그 회차는 🟠 Block·재실행이며 🔴 로 올리지 않는다). 월말·KST 월 1일 자정 직후처럼 **결함이 드러나는 좁은 구간을 지목할 수 있어야** 시드를 충족한다 | `BAT-02_007`·`BAT-02_016`·`BAT-02_017` · `BAT-06_007`·`BAT-06_008`(두 TC 는 본 절차를 "시각 제어 수단"으로 부른다) |
