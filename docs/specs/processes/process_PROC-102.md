...
- **사전 검증**: 조회 조건(검색어·활성 필터) 길이·형식(타입) 재검증(FN-005). 상세는 id UUID 형식. 허용 문자 재검증은 미요구 — 주입 패턴 keyword 도 파라미터 바인딩(SEC-004-02)으로 안전 처리되어 정상 조회(200)한다.
...
B1. 진입 가드 → 인증 → 조회 조건 검증
  ...
  입력 검증: FN-005_validateInput(query|params, schema, rawSize)
             길이·형식(타입) 위반 → 400 EX-SEC-004 (허용 문자·주입 패턴은 미검증 — 바인딩 방어)
...
| EX-SEC-004 | 조회 조건 형식(타입)·길이 위반(허용 문자·주입 패턴은 거부 트리거 아님 — 바인딩 방어) | 조회 거부 | 400 입력 형식이 올바르지 않습니다. |
