...
| 정책(policy) | `BIZ-002-01`·`BIZ-002-03`(`B2`·`B3`)·`BIZ-002-04`(`B3`) · `BIZ-001-01`·`BIZ-001-04`(`B4`) · `BIZ-005-02` ①(`B3`)·`BIZ-005-04`(`B4`) · `DATA-002-01` ①(`B5`) · `DATA-004-01`(`B3`)·`DATA-004-02`(`B2`) · `DATA-001-01`(`B3`~`B6`) · `DATA-002-05`·`BIZ-002-05`(`B2` 조회 판정 · **`B5` 경합 삭제 → 404 `EX-DATA-001`**) · `OPS-004-01`(`B2`~`B6` 질의 실행)·`OPS-004-03`(상위 경계에 참여 — 커넥션 소실은 상위의 500 `EX-BIZ-003` 으로 귀결되며 본 프로세스가 새 코드를 만들지 않는다)·`OPS-004-04`(로그 기록 — FN-015) | 적용 규칙·지점 |
...
C2. 결과 수령
  ...
  EX-DATA-001 이 올라오면 (CONFIRM_RESULT 계기에서만) 상위가 트랜잭션을 되돌리고 404 로 응답한다
  // 되돌릴 변경이 없다(갱신 0행) — 사전 조회 시점의 값으로 200 을 만들지 않는다 (POL DATA-002-05)
```
...
B5. 결과 확인 표시 — FN-010 · POL DATA-002-01 ① · BR-010 (트랜잭션 참여)
  ...
  lookup = B2 재수행                              // 갱신 0행의 원인을 가른다 (같은 경계에서 재조회)
  if (lookup.branch == 'NONE')                    → throw EX-DATA-001 (404)
      // 상위 사전 조회와 이 갱신 사이에 보관 배치(PROC-304 B5)가 하드 삭제했다 — 경합 삭제.
      //   저장 실패가 아니라 대상 없음이다 (POL DATA-002-05 · BIZ-002-05)
      // 이 계기(CONFIRM_RESULT)만 404 다 — B4·B6 의 갱신 0행 + NONE 은 EX-BIZ-003(500) 이다.
      //   여기는 조회 요청의 응답을 만드는 자리라 미진입과 같게 답한다 (되돌릴 변경 없음 —
      //   상위가 경계를 되돌리고 404 로 끝낸다)
  return { confirmedAt: lookup.record.resultConfirmedAt }  // 이미 표시돼 있으면 그 값 (BR-010)
      // 표시 조건 미충족이면 null 이며 그 뜻은 하나뿐이다 (FN-010 §반환값 규약)
  저장 실패 시                                     → throw EX-BIZ-003 (500)
...
| 6 | BE `B5` | 결과 확인 표시 | 응답 시각 | 조건부 UPDATE(결과 확정 행만) · 갱신 0행이면 재조회로 "이미 표시"와 "대상 소실(404)"을 가른다 | 확인 일시 또는 `EX-DATA-001` |
...
| `EX-DATA-001` | **`B5` 갱신 0행 + `B2` 재수행이 `NONE`** — 상위의 사전 조회와 이 갱신 사이에 보관 배치(PROC-304 `B5`)가 하드 삭제(경합) | 되돌릴 변경이 없다(갱신 0행). 호출측 트랜잭션을 되돌리고 **미진입·보관 만료와 구별하지 않는 없음**으로 응답한다(`DATA-002-05`·`BIZ-002-05`) | 404 (상위가 응답 — PROC-201 `B5`) |
| `EX-BIZ-003` | 계기 값 위반 · 저장 실패 · **`B4`·`B6` 의 갱신 대상 소실** | 호출측 트랜잭션과 함께 되돌린다 | 500 (상위가 응답) |

- **본 프로세스가 만드는 4xx 는 `B5` 의 `EX-DATA-001` 하나뿐이다.** 나머지 계기에서 `branch = 'NONE'` 을 404 로 바꿀지는 호출측이 정한다(`BIZ-002-05`). `B5` 만 예외인 이유는 그 계기가 **조회 요청의 응답을 만드는 자리**여서 삭제된 레코드를 미진입과 같게 답해야 하기 때문이며(`DATA-002-05`), 그 판단의 정본은 [`../functions/function_FN-009-011.md`](../functions/function_FN-009-011.md) `FN-010` §반환값 규약이다. `B4`·`B6` 의 같은 형태(갱신 0행 + `NONE`)는 그대로 `EX-BIZ-003`(500)이다.

### 실행 결과

- **정상 결과**: 계기별 반환 값(`branch`·`record`·`isCreated`·일시). 영속화는 계기당 최대 1행이다.
- **실패 결과**: `EX-BIZ-003` — 호출측 트랜잭션이 되돌려지고 지표 갱신도 함께 취소된다. **결과를 확정하지 않는다.** `B5` 의 경합 삭제만 `EX-DATA-001`(404)이며, 이때도 호출측이 경계를 되돌린다 — 되돌릴 변경은 없다(갱신 0행).
...
- **`B5` 의 갱신 0행을 한 가지 원인으로 뭉뚱그리지 않는다.** 같은 경계에서 재조회해 "이미 표시됨"(기존 값 반환)과 "대상 소실"(404 `EX-DATA-001`)을 가른다 — 재조회 없이 값을 돌려주면 **삭제된 레코드에 200 응답이 나간다**. `B4`·`B6` 의 같은 형태는 500 이라는 점과 함께 기억한다(계기마다 응답이 다르다).
