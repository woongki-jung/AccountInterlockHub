...
- **정의 대상**: 연동 1건의 진행과 결과를 **추적 키 기준 단일 레코드 하나에 모으는** 처리다. 조회·생성·이어쓰기·결과 확정·결과 확인 표시·완료 콜백 기록이라는 **여섯 계기**를 한 프로세스가 받아 트랜잭션 경계와 조건부 갱신 형태를 통일한다.
- **관련 PRD 요구사항**:
...
B4. 결과 확정·기록 — FN-009 · POL BIZ-001-04 · BIZ-004-03 (트랜잭션 참여)
  ...
  if (갱신 행 수 == 1)
      PROC-303({ kind: 'RESULT', resultCode, at })    // 해당 카운터 +1 · 같은 트랜잭션
  else
      lookup = B2 재수행
...
|---|--------|--------|--------------|----------------|---------------|
| 1 | 호출측 `C1` | 기록 계기 전달 | (상위 판단) | `kind`·추적 키·시각 전달 · 트랜잭션 공유 | 계기 |
