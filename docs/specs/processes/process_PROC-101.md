...
- **사전 검증**: 본문 크기 ≤1MB(FN-005 SEC-004-03), MDL-101 스키마(필수·길이·형식(타입)) 재검증(FN-005 SEC-004-01). 주입 방어는 파라미터 바인딩(SEC-004-02) 단독 — 허용 문자 재검증은 미요구(주입 문자 입력도 스키마 통과 시 저장, 200). 업무 규칙(URL 형식·동의 항목 1개 이상·키값 지정)은 FN-006 이 재검증.
