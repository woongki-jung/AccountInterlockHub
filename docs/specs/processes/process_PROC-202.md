...
F2. 응답 수신 → 결과 전이   (→ SCR-006 / 재입력)
  ...
  onError(err):
    if (err.code=='EX-SEC-006') → 본 화면 유지 + 생년월일 인라인 에러("사용자 정보가 일치하지 않습니다.") + 재입력·재제출 허용(하드 잠금 없음)   // BR-204 (복호화 실패 = 서버 판정, FE 형식 에러와 문구 구분)
    else if (err.code=='EX-SEC-007' || err.code=='EX-BIZ-008') → navigate SCR-006(링크 오류 — 발송처 문의, 재입력 불가)
    else if (err.code=='EX-BIZ-004') → 본 화면 유지 + 전달 실패 Banner("동의 처리에 실패했습니다. 다시 시도해주세요.") + 승인 재제출(재승인) 허용   // SCR-006 이동 없이 SCR-005 재시도, 상태·이력은 저장됨
...
| 응답→FE | FE 어댑터 | 응답·에러 | SCR-006 상태 / SCR-005 유지 | 완료·취소·링크오류→SCR-006 유형 매핑, 복호화 실패·전달 실패→SCR-005 유지(재입력·재시도) |
...
| 5 | FE | 결과 전이 | 응답/에러 | 완료·취소·링크오류→SCR-006 표시 / 복호화 실패·전달 실패→SCR-005 유지(재입력·재시도) | (UI 전이) |
...
| EX-SEC-006 | (PROC-203 전파) 복호화 실패(생년월일 불일치·패딩·키 불일치) | SCR-005 유지·생년월일 재입력 재시도(BR-204) | 400 사용자 정보가 일치하지 않습니다. |
...
| EX-BIZ-004 | (PROC-203 전파) 수신처 B 전달 실패(재시도 2회 후) | SCR-005 유지·전달 실패 Banner·승인 재제출(재승인) 허용(처리 상태·연동이력 저장됨) | 502 연동 대상 처리에 실패했습니다.(화면 알림 "동의 처리에 실패했습니다. 다시 시도해주세요.") |
...
- **실패 결과**: EX-SEC-006(400, SCR-005 재입력)·EX-SEC-007/EX-BIZ-008(400, SCR-006 링크 오류)·EX-BIZ-004(502, SCR-005 유지 재시도·상태·이력 저장됨). 링크 오류만 SCR-006 로 종료 안내하고, 복호화 실패·전달 실패는 SCR-005 에서 재시도한다.
...
- 승인 경로의 복호화 실패(EX-SEC-006)는 SCR-005 에 머무르며 생년월일 재입력·재제출을 허용한다(하드 잠금 없음, AUTH-004-02). 재제출은 동일 접근 주소·encX·encY 에 생년월일만 갱신해 다시 POST 한다(암호값 재수신 불요). 남용은 요청 제한(OPS-001)이 1차 억제한다. 복호화 실패 시 인라인 문구는 "사용자 정보가 일치하지 않습니다."로 고정하며, FE 형식 위반 안내("생년월일 6자리(YYMMDD)를 정확히 입력해 주세요.")와 구분해 노출한다.
- 전달 실패(EX-BIZ-004)도 별도 결과 화면(SCR-006)으로 이동하지 않고 SCR-005 에 머무르며 전달 실패 알림("동의 처리에 실패했습니다. 다시 시도해주세요.")을 노출하고 승인 재제출(재승인)을 허용한다. 복호화 성공 이후라 처리 상태·연동이력 1건은 이미 저장돼 있으므로 재제출은 새 승인으로 처리된다(추적 키 재사용은 최신 1건 tiebreaker 로 정리, EXC-BIZ-12). 링크 오류(EX-SEC-007·EX-BIZ-008)만 SCR-006 로 종료 안내한다.
