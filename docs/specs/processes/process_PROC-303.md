...
B3. 입력 검증 — FN-005_validateInput(raw{trackingKey}, callbackSchema, rawSize)  (SEC-004)
  ...
  trackingKey: NotBlank·MaxLength(255) 위반 → 400 EX-SEC-004
  // 주입 패턴은 파라미터 바인딩 단독 방어(SEC-004-01/02) — 별도 재검증 없이 비매칭 일반값으로 통과, B4 에서 대상 미특정이면 404 EX-BIZ-006
...
| EX-SEC-004 | 필수 항목 누락·형식(타입)·크기 위반(주입 패턴은 파라미터 바인딩 단독 방어 — SEC-004-01, 비매칭 일반값 통과 후 404 EX-BIZ-006) | 콜백 거부 | 400 요청이 올바르지 않습니다. |
