...
- **정의 대상**: 연동 관리자가 등록된 연동 구성의 활성/비활성 상태를 전환하는 프로세스. SVC-002·SCR-002/004 가 PROC-102 로 느슨히 인용했으나 단일 책임·트리거 원칙에 따라 별도 채번한다. 활성 상태는 성과 지표 "활성 연동 구성 수"의 근거다.
...
- **참조 정합**: SCR-002/004 의 활성 `Toggle` 인터랙션과 SVC-002 BR-103 은 본 PROC-105 를 인용해야 한다(기존 PROC-102 인용 정정 대상).
