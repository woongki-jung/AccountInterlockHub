...
| InlineAlert | SCR-001 · SCR-002 | 단계를 바꾸지 않는 오류·안내 |
...
- **상태**: 미선택 / 선택(체크 표시 `--color-primary`) / 포커스(행에 포커스 링) / 잠금(제출 중).
- **접근성**: 네이티브 `<input type="checkbox">` + `<label>` 조합. 목록은 `<ul>`·항목은 `<li>`. 필수 항목에는 `필수` 라는 **글자**를 함께 둔다(별표 하나로 대신하지 않는다).
...
- **위치**: 필드에 매인 오류는 해당 필드 아래, 화면 전체에 걸린 오류는 카드 제목 아래.
