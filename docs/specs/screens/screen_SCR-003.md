...
- **화면 목적**: 연동 관리자가 서비스 A↔서비스 B 연동 1건을 코드 개발 없이 화면 구성만으로 등록·편집한다. 서비스 A 호출 주소·전달 파라미터 정의·사용자 동의 항목·서비스 B 전달 주소를 입력하며, 동의 항목·파라미터는 동적 반복 행으로 여러 개 입력한다. 전달 파라미터 중 하나를 **사용자 키값 파라미터로 명시 지정**할 수 있다(선택 입력·구성당 최대 1개 — 지정 시 연동이력·완료 확인/콜백의 사용자 키값 근거가 된다).
...
- 전달 파라미터 각 반복 행은 파라미터명·원천 키명·서비스 B 전달 여부에 더해 '사용자 키값' 단일 선택(라디오)을 둔다. 라디오는 전달 파라미터 반복 행 전체를 하나의 그룹으로 묶어 **최대 1개 행만 선택**되며(구성당 최대 1개, BIZ-001-07), 기본값은 '지정 없음'(전 행 미선택)이다. 지정은 **선택 입력**이며(EXC-BIZ-09), 지정한 파라미터의 진입 값이 연동이력·완료 확인·완료 콜백의 사용자 키값으로 사용된다(BIZ-004-05). 파라미터 정의 위(또는 우측)에 '사용자 키값 지정 없음' 컨트롤을 두어 지정을 해제(미지정)할 수 있다. 라디오는 실재하는 파라미터 행에 귀속되므로 "지정 대상은 이미 입력된 파라미터 중 하나"가 구조적으로 보장된다.
...
| 사용자 키값 파라미터 지정 | MDL-101 | parameters[].isUserKey | 라디오 단일 선택(전 행 그룹, 미지정 허용) | 마스킹 없음(파라미터 정의명일 뿐 값 아님 — 노출 무방) |
...
| 클릭 | 행 삭제 | 반복 행 제거 | (없음, 클라이언트) | (트리거 없음) | 행 제거(삭제 행이 사용자 키값 지정이면 지정 자동 해제) |
| 클릭 | 파라미터 행 "사용자 키값" 라디오 | 지정 파라미터 단일 선택 | (없음, 클라이언트) | (트리거 없음) | 선택 행만 `isUserKey=true`, 나머지 행 `false` 로 전환 |
| 클릭 | "사용자 키값 지정 없음" | 지정 해제(미지정) | (없음, 클라이언트) | (트리거 없음) | 전 행 `isUserKey=false`(미지정, 선택 입력) |
...
| 사용자 키값 파라미터 지정(행) | radio(전 행 단일 선택 그룹) | MDL-101.parameters[].isUserKey | N | `parameters.filter(p => p.isUserKey).length <= 1`(라디오 그룹이 구조적 보장, BIZ-001-07) · 미지정(전 행 false) 허용(EXC-BIZ-09) · 지정 행은 실재 파라미터에 귀속(라디오 구조상 보장) | (선택 입력 — 다중·미실재 지정은 라디오 구조상 발생 불가하므로 별도 인라인 에러 없음. 지정 행의 파라미터명 공백은 파라미터명 필드 검증이 담당) |
...
> 사용자 키값 지정(isUserKey)은 서버 검증 FN-006 step 4(BIZ-001-07)와 정합한다 — 서버는 `parameters.filter(p => p.isUserKey).length > 1`(다중 지정) 또는 지정 파라미터 미실재를 422 EX-BIZ-001 로 거부한다. FE 는 라디오 단일 선택으로 다중 지정을, 라디오의 행 귀속으로 미실재 지정을 구조적으로 차단하여 정상 흐름에서 이 422 가 발생하지 않게 한다(1차 방어). 지정 행 삭제 시 지정을 자동 해제해 "지정 유지 상태의 파라미터 삭제 제출"(서버 422)을 사전 차단한다. 지정 파라미터의 진입 값 누락은 등록·편집이 아닌 진입 시점(FN-016 EX-BIZ-007) 검증이므로 본 폼의 책임 밖이다.
...
| "사용자 키값 지정 없음" 컨트롤 활성 | 어떤 파라미터 행의 `isUserKey`가 선택된 상태 | BIZ-001-07(선택 입력 — 미지정 복귀 허용) |
...
- 사용자 키값 지정은 전달 파라미터 반복 행 전체를 단일 라디오 그룹으로 묶어 상태(선택된 행 인덱스 또는 없음)를 관리한다. 제출 시 선택 행만 `isUserKey=true`, 나머지는 `false` 로 직렬화한다(서버는 `user_key_param_id`=지정 행의 저장 id 로 참조 저장 — 값 복제 없음, MDL-101 매핑). 미지정(전 행 false)은 정상 허용값이다(EXC-BIZ-09). 편집 진입 프리필 시 `user_key_param_id` 와 매칭되는 파라미터 행의 라디오를 선택 상태로 복원한다(MDL-101 ENT→도메인 매핑).
- 지정된 파라미터 행을 삭제하면 지정을 자동으로 '지정 없음'으로 되돌려, 삭제된 파라미터를 참조하는 상태로 제출되지 않게 한다(서버 RESTRICT·EX-BIZ-001 사전 방어, [FN-006](../functions/function_FN-006.md)). 지정 대상을 다른 파라미터로 바꾸려면 그 행의 라디오를 선택하면 되고 별도 해제 조작은 필요 없다(라디오 단일 선택).
