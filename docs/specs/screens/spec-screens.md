...
- 사용자 키값 파라미터 지정(`parameters[].isUserKey`, `accountinterlockhub#33`)은 SCR-003 에서 전달 파라미터 반복 행의 단일 선택 라디오로 입력(선택 입력·구성당 최대 1개, BIZ-001-07)하고 SCR-004 에서 지정 파라미터 정의명을 읽기 전용 표시(미지정="지정 없음")한다. 지정은 파라미터의 **정의명**이지 회원 키값이 아니므로 마스킹 대상이 아니다. 지정 여부가 연동이력·완료 확인/콜백 대상 여부를 결정한다(BIZ-004-05). 클라이언트 상태 입력이라 새 트리거 PROC 는 없고 제출은 PROC-101(SCR-003)에 포함된다.
...
- **사용자 키값 파라미터 지정(승계 EXC-BIZ-09, `accountinterlockhub#33`)**: SCR-003 에서 '선택 입력'(미지정 허용)으로 UI 구성한 것은 상위 확정 기본안을 승계한 것이다. '필수' 전환 여부는 담당자 확정 대기 — 필수 전환 시 SCR-003 에 "사용자 키값 파라미터를 1개 지정해주세요" 저장 차단 검증을 추가해야 한다(현재는 미추가). 지정 UI 형태는 파라미터 행 단일 선택 라디오를 기본안으로 정의(별도 Select 대안 대비 삭제 시 자동 해제·실재 보장이 구조적으로 우수).
