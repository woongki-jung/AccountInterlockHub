...
- 사용자 키값 파라미터 지정(`parameters[].isUserKey`, `accountinterlockhub#33`)은 SCR-003 에서 전달 파라미터 반복 행의 단일 선택 라디오로 입력(필수·구성당 정확히 1개·반드시 1개 선택, BIZ-001-07)하고 SCR-004 에서 지정 파라미터 정의명을 읽기 전용 표시한다. 지정은 파라미터의 **정의명**이지 회원 키값이 아니므로 마스킹 대상이 아니다. 지정이 필수라 정상 구성은 항상 연동이력·완료 확인/콜백 대상이 된다('미지정' 분기는 방어적 — BIZ-004-05). 클라이언트 상태 입력이라 새 트리거 PROC 는 없고 제출은 PROC-101(SCR-003)에 포함된다.
...
- **사용자 키값 파라미터 지정(EXC-BIZ-09 필수 확정, `accountinterlockhub#33`, 2026-07-08)**: 사용자 키값 파라미터 지정은 **필수(정확히 1개)**로 확정됐다 — SCR-003 은 파라미터 행 단일 선택 라디오에서 **반드시 1개를 선택**해야 하고, 미선택(0개) 저장 시 "사용자 키값 파라미터를 1개 지정해주세요" 검증으로 저장을 차단한다(FN-006 step4 EX-BIZ-001 의 1차 방어). '사용자 키값 지정 없음' 컨트롤은 제거했다. 지정 UI 형태는 파라미터 행 단일 선택 라디오(삭제 시 재지정 유도·실재 보장이 구조적으로 우수).
