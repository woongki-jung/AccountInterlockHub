...
| `Badge` | active(초록)·inactive(회색)·완료(success)·거부(info 중립)·링크오류(error) | 활성 여부·연동 결과 상태 표기(SCR-006 결과 유형). 전달 실패는 `#215` 로 SCR-005 의 `Banner`(재시도 알림)로 이관 |
...
| 전달 실패(재시도) | error `Banner` + "동의 처리에 실패했습니다. 다시 시도해주세요." + 승인 재제출(재승인) 허용(화면 유지) | SCR-005 | EX-BIZ-004(`#215`) |
