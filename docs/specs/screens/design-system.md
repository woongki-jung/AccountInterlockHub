...
| `Badge` | active(초록)·inactive(회색)·완료(success)·거부(info 중립)·전달실패(warning/error)·링크오류(error) | 활성 여부·연동 결과 상태 표기(SCR-006 결과 유형) |
...
| 상태 | 표현 | 화면 | 관련 코드 |
|------|------|------|-----------|
| 본인확인 입력 | 생년월일 `TextField(birthdate)` + 안내 캡션 | SCR-005 | AUTH-004 |
| 제출 대기(Submitting) | 승인/거부 버튼 Spinner + 폼 `disabled` | SCR-005 | PROC-202 |
| 복호화 실패·재입력 | 생년월일 필드 인라인 에러(`role=alert`) + 필드 재활성·재제출 허용(화면 유지) | SCR-005 | EX-SEC-006(BR-204) |
| 링크 오류(재입력 불가) | error `Badge` + 발송처 문의 안내(입력 폼 없음) | SCR-006 | EX-SEC-007·EX-BIZ-008 |
| 전달 실패(재시도) | error `Banner` + "잠시 후 다시 시도" 안내·재진입 경로 | SCR-006 | EX-BIZ-004 |
| 연동 완료 | success `Badge` + 완료 안내(원문·회원 키·추적 키 미포함) | SCR-006 | BIZ-003-05·SEC-007-02 |
| 거부(취소) | info(중립) `Badge` + 취소 안내 | SCR-006 | BIZ-002-07(EXC-BIZ-03) |
| 결과 없음(Fallback) | 중립 안내 + 발송처 재진입 유도 | SCR-006 | (표시 규칙) |

- 복호화 실패는 사용자 정정 가능이라 **화면을 이탈하지 않고** 재입력을 유도하고, 발송처 데이터·링크 오류(형식·추적 키 누락)는 재입력이 무의미하므로 결과 화면으로 종료 안내한다(EXC-BIZ-13).
