...
- **`LOOKUP` 계기의 호출만 `exec` 없이 부른다.** 그 호출 지점(PROC-201 `B3` · PROC-202 `B3` · PROC-203 `B3` → PROC-301 `B2`)은 호출측이 아직 경계를 열지 않은 자리라 **넘길 실행 문맥이 없다** — 인자 없음은 표기 누락이 아니라 사양이며, `exec` 를 나중에 채워 넣거나 필수 인자로 바꾸지 않는다([`../processes/process_PROC-301.md`](../processes/process_PROC-301.md) §진입점 및 진입 조건).
...
5. 지표 요청 수 계수 — FN-013 · POL BIZ-005-02 ① (같은 트랜잭션 경계)
   FN-013({ kind: 'REQUEST', at }, exec)     // exec 와 같은 실행 문맥에서 수행한다
                                             // 받은 exec 를 두 번째 인자로 그대로 넘긴다 —
                                             //   이 전달이 참여의 성립 조건이다 (FN-013 §시그니처)
