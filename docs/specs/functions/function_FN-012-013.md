...
- **전달 경로는 `PROC-103` `B6` → `PROC-302` → 본 기능 한 줄이다.** `B6` 이 연 커넥션·실행자를 `PROC-302` 가 `exec` 로 받아 **그대로** 본 기능에 넘긴다([`../processes/process_PROC-302.md`](../processes/process_PROC-302.md) §입력/출력 정의). 중간 어느 지점에서든 새로 커넥션을 얻으면 `B6` 이 추적 레코드 행에 건 `FOR UPDATE` 와 갈려 동시 승인 직렬화가 무너지고, **같은 경계에서 수행한 전달 시도 표지 검사(기존 증적 유무)도 무의미해진다** — 검사 결과와 INSERT 가 다른 경계에 놓이면 그 사이에 상대 요청이 끼어들 수 있다.
...
- **증적 기록과 승인 확정을 같은 처리 경계에 둔다**(`BIZ-003-04`). 증적 없는 승인이나 승인 없는 증적이 생기지 않게 한다. **그 경계는 호출측이 열고 닫는다** — 본 기능은 `exec` 로 받은 실행자 위에서 실행만 한다(§시그니처).
- **중복 증적을 막는 것은 본 기능이 아니라 호출측이다.** 같은 추적 키의 동시 승인은 호출측([`PROC-103`](../processes/process_PROC-103.md) `B6`)이 **추적 레코드 행을 잠그고 그 잠금 안에서 이번 레코드의 증적 유무를 확인**해 걸러 낸다 — 본 기능은 호출됐다는 것 자체를 "만들어도 되는 상태"로 신뢰하고 INSERT 한다. **행 잠금만으로는 막히지 않으며**(잠금은 순서만 세운다) 증적 테이블에는 유일 제약도 없다([`data_ENT-002.md`](../datas/data_ENT-002.md) §관계 정의 — 보관 주기를 넘어 같은 키의 증적이 공존하므로 둘 수 없다). **여기에 존재 검사를 옮겨 넣지 않는다** — 검사는 잠금을 쥔 자리에서 해야 성립하고, 본 기능은 경계를 열지 않아 그 자리가 아니다.
