...
- 감사 로그는 애플리케이션 로그와 분리 가능한 채널로 남기고, 기록 직전 FN-010 마스킹을 일괄 적용한다. append-only 특성상 수정하지 않는다. 저장은 bigint identity PK 의 append 삽입으로 PostgreSQL 에 이미 최적인 패턴이라 현행 규모에서 추가 인덱스·파티셔닝을 두지 않으며, 시간 순 조회가 필요하면 힙의 물리 저장 순서에 의존하지 않고 ORDER BY(id 또는 occurred_at)로 논리적으로 보장한다([ENT-006](../datas/data_ENT-006.md) §구현 가이드 확정).
- 기록은 best-effort 로 처리해 감사 실패가 업무 처리를 차단하지 않게 한다. 보존 초과분 삭제 배치·PROC 는 MVP 미정의이며, 도입 시 occurred_at BRIN 인덱스 또는 월별 range 파티셔닝(1순위 후보, ENT-006)과 삭제 PROC 를 spec 리비전으로 함께 확정·채번한다.
