...
| 연관 정책 | `DATA-002-01` · `OPS-002-04` · `BIZ-002-04` |
...
| HTTP status | EX 코드 | 발생 조건 | 사용자 메시지 | 개발자 노트 |
|-------------|---------|-----------|---------------|-------------|
| 500 | `EX-BIZ-003` | 표시 갱신 실패(저장 오류) | "처리 중 문제가 발생했습니다. 잠시 후 다시 시도해 주세요." | 응답도 내보내지 않는다 — 응답은 갔는데 표시가 남지 않는 상태를 만들지 않는다(`MDL-012` 구현 가이드) |
