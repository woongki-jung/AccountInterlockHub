...
- **ai-pm 오케스트레이션 전략**: [`ai/strategies/ai-pm.md`](ai-pm.md)
	- Redmine 작업세션 이슈를 폴링해 담당자 지시를 인지하고 필요한 작업을 서브에이전트로 디스패치하며, 결과를 작업세션 이슈 노트·상태로 회신하는 마스터 세션 ai-pm 을 정의. 소통·작업 기록 정본은 Redmine 일감(작업세션 이슈 = 대화 스레드이며 담당자만 연다, 봇은 그 이슈에 댓글만). 모든 산출 일감은 **작업세션 이슈를 뿌리로 하는 계층**에 두며 프로젝트 루트에 만들지 않는다([`work-tracking.md`](work-tracking.md) §계층·연관). 서브에이전트의 담당자 질의·승인은 ai-pm 릴레이(§질의·승인 릴레이)로 처리.
