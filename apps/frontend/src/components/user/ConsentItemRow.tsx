 ...
 * 히트 영역 = 항목 행 전체 — 체크박스+라벨+설명을 하나의 label 로 묶고 align-self:stretch 로 부모
 * .row 의 렌더 높이([상세] 없는 행 70px·있는 행 72px, 산출 근거는 ConsentItemRow.module.css 의 .main
 * 주석)를 그대로 채워 어디를 눌러도 토글되게 한다(Checkbox(user) 최소 48px·§사용자 화면 접근성
 * 44x44px 모두 충족, Phase 3 이월-2). [상세] 버튼은 label 바깥의 형제 요소로 둬 클릭이 체크 토글과
 * 분리되게 한다(전파 차단 트릭 불필요 — DOM 상 label 의 자식이 아니므로 자연히 분리된다).
