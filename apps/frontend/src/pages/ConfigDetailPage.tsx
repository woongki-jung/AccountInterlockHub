/*
 * SCR-004 발송처 접근 주소 구성 상세 화면 — PROC-102(상세 조회) / PROC-105(활성 전환) / PROC-106(삭제) / SVC-002 / ADM-02.
 ...
 *  - Card 묶음: 기본 정보(고유 ID·구성명·활성·생성 일시·동의 대상 설명 문구, 설정된 경우만) → 수신처 B 전달
 *    (전달 주소·전달 방식) → 동의 항목(테이블, 약관 컨텐츠 있는 행은 [약관 보기]로 콘텐츠 Modal 열람).
 *  - `#214`(P3) 로 서비스 A 진입 주소·전달 파라미터 목록·사용자 키값 지정 표시를 제거했다(EXC-BIZ-14). `#215`
 *    로 동의 대상 설명 문구(consentNotice) 표시를 추가했다(BIZ-002-08, 미설정 시 행 생략).
...
import type { ConfigDetail, ConsentItemResponse } from '../lib/configApi';
  ...
  return (
    <AdminShell>
      <nav className={styles.crumb}>
        <a href={CONFIGS_LIST_PATH}>발송처 접근 주소 목록</a> / 상세
      </nav>

      <div className={styles.dethead}>
        <h1 className={styles.title}>
          {loadState === 'loaded' && detail ? detail.configName : '발송처 접근 주소 상세'}
        </h1>
      ...
      {loadState === 'notfound' && (
        <EmptyState
          ...
          description="이미 삭제되었거나 존재하지 않는 발송처 접근 주소 구성입니다."
          action={
        ...
        <>
          ...
          <Card className={styles.card}>
            ...
            <dl className={styles.dl}>
              ...
              {detail.consentNotice && (
                <>
                  <dt>동의 대상 설명 문구</dt>
                  <dd className={styles.notice}>{detail.consentNotice}</dd>
                </>
              )}
            </dl>
          </Card>
          ...
          {/* (2) 수신처 B 전달 */}
          <Card className={styles.card}>
            <h2 className={styles.cardTitle}>수신처 B 전달</h2>
            <dl className={styles.dl}>
              <dt>전달 주소</dt>
              <dd>
      ...
      >
        <strong>{detail?.configName}</strong> 을(를) 삭제하시겠습니까? 이 작업은 되돌릴 수 없으며 감사
        로그에 기록됩니다(OPS-002).
      </Modal>
