...
/**
 * 서비스 대면 API 요청 제한 카운터(FN-014 / OPS-001) — 인증 주체(FN-004 caller) 기준 분당 요청 수를
 * 세는 인메모리 고정 윈도우 카운터. 단일 App Service 기준 싱글턴(스케일아웃 시 공유 캐시 전환 여지).
 *
 * 만료 회수: 카운터 키는 `${scope}:${caller.id}` 로 인증 통과 주체(A/B, 유계)만 유입되어 실제 누수
 * 위험은 작지만, EntryRateLimitStore 와 동일한 정리 규율(윈도우 주기 sweep + unref + OnModuleDestroy)을
 * 준용해 만료 엔트리를 상시 회수한다(코드 안티패턴 예방). 타이머는 프로세스 종료를 붙잡지 않도록 unref 한다.
 */
@Injectable()
export class ServiceApiRateLimitStore implements OnModuleDestroy {
   ...
   * 어차피 리셋할 대상이므로 삭제해도 요청제한 판정에 영향이 없다(재요청 시 새 윈도우로 통과).
   */
