 ...
 * 요청제한(FN-014)·입력 검증(FN-005)은 상위(가드·ValidationPipe)가 선행하고, 본 서비스는 **연동 추적 키
 * 단독** 스코프의 완료 콜백 미수신 최신 이력 1건을 특정해 완료 콜백 수신 여부=수신·수신 일시를
 * 연동이력(ENT-007)에만 기록한다(`#214` BIZ-004-09).
 *
 ...
 *    (BR-303·EXC-BIZ-10). 대상 미특정(스코프 내 이력 자체 없음)만 단일 404 EX-BIZ-006 으로 응답한다
 *    (존재 여부 비노출).
 *  - 완료 기록 UPDATE 는 `WHERE id = ... AND callback_received = false` 조건절이 곧 동시성 가드다 — 영향
 *    행 0건(UPDATE...RETURNING 튜플의 안쪽 행배열이 비어 있음, firstUpdatedRow 로 판별)이면 재통지·동시
 *    콜백으로 보고 멱등 성공한다.
 *
 * **처리상태 불변경(BIZ-004-11)** — 처리상태(ENT-004, TBL_INTERLOCK_PROCESS_STATUS) 4항목을 변경하는 어떤
 * SQL 도 두지 않는다. 본 서비스의 유일한 갱신 대상은 TBL_INTERLOCK_HISTORY(연동이력)뿐이다. 감사·오류 응답에
 * 전달받은 연동 추적 키 원문을 남기지 않는다(FN-010 마스킹, SEC-005-04).
 *
 * DB 접근은 파라미터 바인딩만 사용한다(SEC-004-02).
 */

// 완료 기록 UPDATE 의 RETURNING 결과 행(영향 행 수 판정용). id 는 내부 surrogate PK(ENT-007.id).
...
export class CallbackService {
   ...
   * @param callback 완료 콜백 요청(MDL-305, { trackingKey }) — 형식 검증(FN-005) 통과값.
   * @param now      완료 콜백 수신 일시(callback_received_at).
   * @throws AppException EX-BIZ-006(404) 스코프 내 이력 없음(단일 응답, 존재 여부 비노출).
   */
  async recordCompletionCallback(callback: CallbackDto, now: Date): Promise<void> {
    const { trackingKey } = callback;

    // 1. 스코프 해석(FN-019, pendingOnly=true — 콜백 특정은 미수신 최신 1건, BIZ-004-09).
    const res = await this.historyScopeService.resolveHistoryScope(trackingKey, true);

    // 2. 미수신 최신 이력 없음 — 완료 이력만 존재(재통지 멱등) / 스코프 내 이력 자체 없음(404) 분기(BR-303).
    if (res.target == null) {
      if (res.anyInScope) {
    ...
    // 3. 완료 기록(내부 PROC-403 단건 UPDATE, 동시성 가드) — 연동이력만 대상(처리상태 ENT-004 불변경, BIZ-004-11).
    ...
    const result = await this.dataSource.query(
      `UPDATE "TBL_INTERLOCK_HISTORY"
        ...
        WHERE id = $2 AND callback_received = false
        RETURNING id`, // surrogate id 대상 + 동시성 가드(callback_received=false) + 영향 행 판정
      [now, res.target.id],
    );
    ...
    // 4. 콜백 수신 감사(CALLBACK_RECORDED) — trackingKey 는 FN-010 마스킹, 원문 미기록(SEC-005-04).
    await this.auditService.write({
