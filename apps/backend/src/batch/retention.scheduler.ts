...
import { resolveAbsoluteDays, resolveConfirmedDays, resolveCron } from './retention.env';
import { RetentionService } from './retention.service';

/**
 * 보관정책 배치 스케줄러 — PROC-402 진입점(앱 내부 스케줄, 일 1회 도래). 얇은 래퍼로, 실제 삭제 로직은
 * RetentionService(FN-011)에 위임한다. cron·보관 기간(fallback two-pass)은 환경변수(RETENTION_CRON 기본
 * '0 3 * * *', RETENTION_CONFIRMED_DAYS 기본 90, RETENTION_ABSOLUTE_DAYS 기본 180)로 재정의한다.
 *
...
export class RetentionScheduler {
  ...
  async handleRetentionCron(): Promise<void> {
    ...
    try {
      const result = await this.retentionService.runRetentionBatch(new Date(), confirmedDays, absoluteDays);
