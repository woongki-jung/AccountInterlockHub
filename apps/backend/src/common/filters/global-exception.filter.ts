...
// 본문 없는 일반 404(SEC-003-02)를 한 곳에서 처리한다.
 ...
 * 자신이 매치 실패 시 내부적으로 던지는 `NotFoundException`도 함께 받는다.** Nest 는 아무
 * 라우트도 매치하지 못하면 `@nestjs/core` `RoutesResolver.registerNotFoundHandler()` 가
 ...
 *      → FN-014 엔벨로프.
 *   2) 그 밖의 `NotFoundException`(= 위에서 설명한 라우팅 자체의 매치 실패) → 본문 없는 일반
 *      404(EX 코드 없음). 자가진단 경로의 메서드 불일치도 이 분기로 떨어진다 — route-guard
 *      미들웨어가 자가진단 경로를 405 판정 표에서 의도적으로 제외해 두었기 때문이다
 *      (`known-routes.ts` 참고).
 *   3) 그 밖의 모든 예외(카탈로그 어떤 코드로도 분류되지 않음) → `EX-OPS-002` 500(마지막 방어).
 *
 * 어떤 분기도 원본 예외의 `message`·`stack`·`cause` 를 응답에 옮기지 않는다(`SEC-002-05`·
 * `DATA-001-04`) — 1)의 `message` 는 항상 카탈로그 기본값이고, 2)·3)은 응답 본문에 원본 정보를
 * 아예 담지 않는다.
 */
@Catch()
