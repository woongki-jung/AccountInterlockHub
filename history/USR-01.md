...
|---|---|---|---|---|
| 2026-07-08 11:20 | build | `apps/backend/src/user/interlock/interlock.service.ts` FN-016 정정(USR-P2 중 발견) — 지정 사용자 키값 추출 키 `param_name`→`source_key_a`(ENT-003 §구현 가이드 정합). param_name≠source_key_a 지정 구성에서 진입 값 미검출로 오검출되던 것을 정정, 진입(PROC-201)→전달(PROC-203, source_key_a 기준 정합) end-to-end 정합 복원. 인접 주석 정정. `npm run build` 0건. 작성만(리뷰·기능검증 대기) | `accountinterlockhub#45`·`#46` | 🚧 |
| 2026-07-08 10:15 | build | `user/consent/{consent.controller,consent.service,consent.module}.ts`·`consent/dto/submit-consent.dto.ts` 확장(USR-P2) — POST /api/consent/:requestKey 동의/거부 처리(FN-008 processDecision BR-201): requestKey UUID 검증(EX-SEC-004)·진입 컨텍스트+configCode 매칭(불일치·만료 EX-DATA-002)·활성 구성 특정. REJECT→처리상태 실패 1건 저장·컨텍스트 폐기·CONSENT_REJECT 감사·200(EXC-BIZ-03). AGREE→PROC-203 전달 내부호출·컨텍스트 폐기(성공·실패 공통). `CONSENT_REJECT` 감사 이벤트 추가. `npm run build` 0건. 작성만(리뷰·기능검증 대기) | `accountinterlockhub#46` | 🚧 |
