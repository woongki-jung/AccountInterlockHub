...
|---|---|---|---|---|
| 2026-07-08 11:45 | build | `user/consent/{consent.service,consent.controller}.ts` 개정(USR-P3 BE 사전) — GET /api/consent/:requestKey 응답에 configCode 동봉(FE 동의/거부 제출 MDL-203 `{decision, configCode}` 의 configCode 회신용, PROC-202 ctx.configCode 정합). 응답 형상 `ConsentItemResponse[]`(배열)→`{ configCode: string, items: ConsentItemResponse[] }`(객체 `ConsentViewResponse`)로 변경. configCode 는 구성 식별 메타(개인정보 아님, 화면 미표시·FE 메모리 보유). 동의 항목 필드(label·description·termsContent·required·order)·회원키/요청키 등 민감값 미포함 유지. POST(processDecision) 로직 무변경. `npm run build` 0건 | `accountinterlockhub#47` | 🚧 |
