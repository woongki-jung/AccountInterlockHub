...
|---|---|---|---|---|
| 2026-07-08 06:02 | build | `apps/backend/src/user/interlock/interlock.service.ts` FN-016 연동이력 생성(내부 PROC-403, USR-P1) — 지정 구성 진입 시 TBL_INTERLOCK_HISTORY 1건 INSERT(user_key 원문 무변형·request_key PK·callback_received=false)·지정값 누락/공백 400 EX-BIZ-007(컨텍스트 폐기·이력 미생성·요청키 미반환)·미지정 미기록 BR-203·HISTORY_CREATE 감사(userKey 마스킹). `npm run build` 0건 | `accountinterlockhub#45` | 🚧 |
