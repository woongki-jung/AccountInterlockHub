...
등급 하향은 잠정이며, 아래 신호가 나오면 **해당 에이전트만** 한 등급 되돌린다. 신호는 Redmine 정본에 남으므로 별도 계측이 필요 없다 — 누적 근거는 각 `report` 일감의 **6절 실행 프로파일**(호출 횟수·등급·회귀 횟수)이다([`work-tracking.md`](work-tracking.md) §작업 보고).
