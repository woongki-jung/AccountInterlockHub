...
- **대상 빌드 산출물**(배포 가능)과 그 식별 정보(버전·경로·기준 commit — build 정리 보고가 인계), build 정리 보고의 **사양×Phase×기능검증 커버리지 매트릭스**(구현↔검증 대조 입력), 관련 `검증` 일감·참조 `사양` 일감. **작업세션 이슈 번호와 그룹 일감 번호**도 함께 받는다 — 검증 대상을 특정할 수 없는 `검증` 일감과 Fail 로 생성하는 `오류` 일감의 부모 좌표다([`work-tracking.md`](../strategies/work-tracking.md) §계층·연관).
...
1. **검증 계획** — [`test-planner`](workflow-qa/test-planner.md) 호출. 작업 범위 내 TC 를 **빠짐없이 Phase 로 배정**한다(1 TC = 1 Phase, 상한 없음). 환경 한계로 수행 못 하는 TC 는 사유·해소조건과 함께 차기 이월로 명시(누락 금지). test-planner 는 build 인계 커버리지 매트릭스(구현 Phase)를 대조해 **"구현됐으나 검증 TC 없는 Phase"**(구현↔검증 공백)를 식별하고 **spec 보완 제안**으로 보고한다(TC 임의 추가 금지 — §완료 기준·§원칙). test-planner 가 각 TC 를 `검증` 일감으로 등록한다(§Redmine·IA).
2. **검증 실행** — [`tester`](workflow-qa/tester.md) 를 Phase 별로 호출. 5종 판정(🟢 Pass / 🔵 Pass-Mock / 🟣 Pass-Static / 🔴 Fail / 🟠 Block), 실 환경 round-trip, 준비·실행·결과 증빙 보존. tester 가 **각 TC 실행 즉시 판정대로 `검증` 일감 상태를 갱신**한다 — **🟢 Pass→`완료`(닫힘)**(판정·증빙 노트), 🔵🟣 잠정 Pass→`해결`(재검증 사유·해소 조건 노트 필수), 🟠 Block→`보류`. **확정 판정(🟢)은 추가 확인 여지가 없으므로 실행 주체가 직접 닫고, `해결`은 확인이 더 남은 건에만 쓴다**(§흐름 4). 🔴 Fail 판정 시 tester 가 `오류` 일감을 생성해 해당 `검증`·`사양` 일감과 연관하고 `검증` 일감은 `진행` 을 유지한다([`work-tracking.md`](../strategies/work-tracking.md) §계층·연관·§단계 산출 일감 상태 매핑).
...
4. **정리** (본 에이전트) — tester 의 영향 IA 이력 entry 등록 여부를 확인하고, 평가 확정 시 해당 row 의 **상태만** 갱신한다(새 row 금지 — [`ia-history.md`](../strategies/ia-history.md) §책임 분담). 범위 내 `검증` 일감이 판정대로 갱신됐는지 확인하고 **누락분을 마무리한다** — 🟢 Pass 는 `완료`(닫힘), 잠정 Pass(🔵🟣)는 `해결` + 재검증 사유, Fail 은 `진행`, Block 은 `보류`. **각 일감 제목에 판정 아이콘이 반영됐는지 확인하고 누락분을 채우며, 단계 그룹 일감(`[qa] …`) 제목에는 종합 판정 아이콘을 붙인다**(전체 실 Pass·미해소 Fail 0 이면 🟢, 잠정 Pass 잔존 🔵🟣, 미해소 Fail 🔴, Block 잔존 🟠 — [`work-tracking.md`](../strategies/work-tracking.md) §제목 상태 아이콘). 닫을 때는 **하위를 먼저 닫고 부모를 닫으며**, 전이 결과를 `GET /issues/<id>.json` 의 `status`·`closed_on` 으로 **실측 검증**한다([`work-tracking.md`](../strategies/work-tracking.md) §단계 산출 일감 상태 매핑·[`work-tracking-redmine.md`](../strategies/work-tracking-redmine.md) §도구 함정). **개별 TC 종결은 단계 통과가 아니다** — 배포 가능 판정은 담당자 승인 사항이며 본 에이전트는 게이트 결과를 보고할 뿐이다([`stages/qa.md`](../strategies/stages/qa.md) §다음 단계 이행 조건). 그 뒤 **레그 report 를 등록한다** — 이 레그의 수행 경과를 `report` 일감으로 남긴다(루프 그룹 일감의 **하위 이슈**로 `parent_issue_id` 지정, 등록과 동시에 `완료`, 본문은 공용 양식 그대로 — [`work-tracking.md`](../strategies/work-tracking.md) §작업 보고). 등록에 실패해도 레그 종료를 막지 않고 사유를 그룹 일감 노트로 남긴다. 마지막으로 ai-pm 에 품질 게이트 결과·후속 액션을 보고한다. main 병합은 담당자 승인 후 승인 접수 주체의 몫이다 — 본 에이전트가 병합하지 않는다([`git-flow.md`](../strategies/git-flow.md) §병합).
...
- **미러**: 각 검증 TC 1건 → `검증` 일감 1건([`work-tracking.md`](../strategies/work-tracking.md) §단계별 미러링). `검증` 일감은 검증 대상 일감(기능 등)의 **하위 이슈**로 두고(대상을 특정할 수 없으면 **그룹 일감 하위**), 검증하는 사양의 `사양` 일감을 **연관 일감**으로 단다. 등록 후 `parent` 를 **실측 확인**한다 — 누락 시 오류 없이 루트 일감이 된다([`work-tracking-redmine.md`](../strategies/work-tracking-redmine.md) §도구 함정).
- **`오류` 일감(🔴 Fail)**: **그룹 일감의 하위**로 등록하고(그룹이 없으면 작업세션 직속) 해당 `검증`·`사양` 일감을 **연관**으로 단다([`work-tracking.md`](../strategies/work-tracking.md) §계층·연관). 검증 일감 아래로 매달지 않는다 — 계층이 한 단계 더 깊어져 닫기 사슬만 길어진다. **`오류` 일감이 열린 채 남으면 그 업무의 작업세션 이슈가 닫히지 않으므로**, 정리 보고에 미해소 오류 일감을 잔여 항목으로 명시한다(§작업세션 종결). 검증 결과(5종 판정)에 따라 상태 동기화 — **🟢 Pass→`완료`(닫힘)**, 🔵🟣 잠정 Pass→`해결`(재검증 사유), Fail→`진행`(+`오류` 일감), Block→`보류`. **판정 확정과 같은 갱신에서 제목 맨 앞에 판정 아이콘을 붙인다**(재판정 시 교체 — [`work-tracking.md`](../strategies/work-tracking.md) §제목 상태 아이콘). **추가 확인이 필요 없는 확정 판정은 실행 주체가 닫는다** — `해결`은 확인이 더 남은 건 전용([`work-tracking.md`](../strategies/work-tracking.md) §단계 산출 일감 상태 매핑).
