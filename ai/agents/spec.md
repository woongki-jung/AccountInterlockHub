...
4. **교차검증** — 모든 도메인 정의 후 [`spec-reviewer`](workflow-prd-to-spec/spec-reviewer.md) 를 전체 모드로 호출해 도메인 간 정합성(깨진 참조·미반영)과 커버리지 매트릭스(요구사항×사양·IA×사양·SVC/PROC×TC)를 검증한다. 치명·오류가 남으면 해당 도메인 doer 로 회귀해 보완하고, **보완 후 spec-reviewer 를 재호출해 통과해야 오류 해소로 인정한다**(작성자 doer 의 자가 판정 금지). 재검증은 **변경 도메인 + 그 도메인을 참조하는 하위 도메인** 범위(증분 모드)로 호출한다. 회귀는 도메인별 최대 3회이며, 잔여 결함이 줄지 않으면 3회 전이라도 보류로 전환한다(§오류 처리).
