...
2. **요구사항 정합성 게이트(재확인)** — [`prd-reviewer`](workflow-prd-review/prd-reviewer.md) 호출. directing 에서 이미 통과했으면 방향 정의서 변경분 중심의 **경량 재확인**, 방향이 바뀌었거나 directing 을 거치지 않았으면 **전체 평가**다. **치명 결함 0** 일 때만 도메인 정의에 진입한다. 결함이 있으면 진입하지 않는다 — 관련 일감에 결함 노트를 남기고 `보류` 로 바꾼 뒤 중간 보고로 종료한다(directing 회귀 여부는 담당자 응답으로 결정 — [`ai-pm.md`](../strategies/ai-pm.md) §질의·승인 릴레이).
3. **도메인별 사양 정의** — §도메인 의존 순서대로 각 도메인 doer 를 1개씩 순차 호출한다. 상위 도메인이 끝난 뒤 하위에 착수한다. 범위에 해당 도메인 변경이 없으면 생략한다. 호출 시 **[`stages/spec.md`](../strategies/stages/spec.md) §도메인 의존 순서의 "필수 입력" 열에 있는 자료만 넘긴다** — PRD 전문은 정책 도메인에만 주고, 하위 도메인에는 선행 도메인 사양을 준다(§입력 범위 규약). 범위 밖 사양·참고 자료는 본문 없이 **경로 목록만** 알린다. 각 doer 는 산출물을 `docs/specs/<도메인>/` 에 쓰고 **자기 `사양` 일감을 등록(없으면 생성)·갱신**한다(§Redmine·IA).
4. **교차검증** — 모든 도메인 정의 후 [`spec-reviewer`](workflow-prd-to-spec/spec-reviewer.md) 를 전체 모드로 호출해 도메인 간 정합성(깨진 참조·미반영)과 커버리지 매트릭스(요구사항×사양·IA×사양·SVC/PROC×TC)를 검증한다. 치명·오류가 남으면 해당 도메인 doer 로 회귀해 보완하고, **보완 후 spec-reviewer 를 재호출해 통과해야 오류 해소로 인정한다**(작성자 doer 의 자가 판정 금지). 회귀 호출은 **증분**이다 — 지적 항목·대상 절·직전 산출물 경로를 넘기고 그 절만 고치게 한다(사양정의서 전면 재작성 금지 — [`agents.md`](../strategies/agents.md) §실행 규칙 — 증분 회귀). 재검증은 **변경 도메인 + 그 도메인을 참조하는 하위 도메인** 범위(증분 모드)로 호출한다. 회귀는 도메인별 최대 3회이며, 잔여 결함이 줄지 않으면 3회 전이라도 보류로 전환한다(§오류 처리).
5. **목업(마무리)** — **범위에 화면(SCR) 도메인 신규·변경이 있을 때** [`mockup-builder`](workflow-mockup/mockup-builder.md) 를 호출해 확정 화면 사양 기반 경량 목업을 만들고, 담당자 리뷰로 사양을 검증한다. 화면 변경이 없으면 목업을 생략하고 사유를 정리 보고에 명시한다(담당자 명시 요청 시 무변경이라도 수행). 담당자 승인은 **ai-pm 릴레이 경유**다 — 승인 요청을 관련 일감 노트로 남기고 상태를 `해결` 로 바꾼 뒤 중간 보고로 종료하며, 응답이 기록된 재디스패치에서 재개한다([`ai-pm.md`](../strategies/ai-pm.md) §질의·승인 릴레이). 리뷰에서 나온 보완은 사양 정의로 회귀해 반영하며, **사양이 바뀌면 4단계 교차검증으로 복귀해 재통과하는 것을 승인 확정의 전제로 삼는다**(목업 = 사양 검증 도구).
6. **정리·완료 게이트** (본 에이전트) — 완료 직전 [`spec-reviewer`](workflow-prd-to-spec/spec-reviewer.md) 를 **전체 모드로 1회 호출**해 증분 재검증이 놓친 횡단 영향과 커버리지 게이트(요구사항·IA·TC 미매핑/미커버/미검증 0)를 최종 확인한다. 결과·진척률·보류 항목을 취합하고, 완료를 막지 않는 🟡 WARNING·요구사항 제안을 처리 분류로 종결한다 — WARNING 은 범위 내·저비용이면 doer 회귀로 자체보완하고 그 외는 담당자 이월, 요구사항 제안은 임의 채택 없이 담당자 판단 이월 목록으로 완료 보고에 포함한다. 각 doer 의 영향 IA 이력 entry 등록 여부를 확인한다 — entry 추가는 doer 몫이며 본 에이전트는 평가 확정 시 해당 row 의 **상태만** 갱신한다(새 row 금지 — [`ia-history.md`](../strategies/ia-history.md) §책임 분담). 기존 사양을 갱신한 건은 연관 `기능`·`검증` 일감을 점검해 재작업·재검증 필요 여부를 노트로 표시하고(갱신 사양별 영향판정 누락 0), 필요 착수분을 완료 보고에 포함한다([`delivery.md`](../strategies/delivery.md) §사양 변경의 역방향 전파). `사양` 일감 상태를 동기화한다. 그 뒤 **레그 report 를 등록한다** — 이 레그의 수행 경과를 `report` 일감으로 남긴다(루프 그룹 일감의 **하위 이슈**로 `parent_issue_id` 지정, 등록과 동시에 `완료`, 본문은 공용 양식 그대로 — [`work-tracking.md`](../strategies/work-tracking.md) §작업 보고). 등록에 실패해도 레그 종료를 막지 않고 사유를 그룹 일감 노트로 남긴다. 마지막으로 ai-pm 에 완료를 보고한다(build 착수 승인 요청 포함). main 병합은 담당자 승인 후 승인 접수 주체의 몫이다 — 본 에이전트가 병합하지 않는다([`git-flow.md`](../strategies/git-flow.md) §병합).
...
[`stages/spec.md`](../strategies/stages/spec.md) §완성도(완료) 기준을 따른다 — 진입 게이트 치명 0 + 도메인 정의 완료 + 교차검증 치명·오류 0(회귀 보완분 평가자 재검증 통과 + 완료 직전 전체 교차검증 1회 통과) + 커버리지 게이트(요구사항·IA·TC 미매핑/미커버/미검증 0) + (화면 변경 시) 목업 담당자 승인 + (기존 사양 갱신 시) 역방향 전파 영향판정 누락 0 + 🟡 WARNING·요구사항 제안 처리 분류 종결. 미통과 항목은 보류로 기록·보고한다.
...
| 교차검증 치명·오류 | 해당 도메인 doer 회귀 후 spec-reviewer 재검증(증분 모드). 회귀 사유·시도 횟수(N회차)·직전 대비 잔여 결함 수는 해당 도메인 `사양` 일감 노트로 기록. **도메인별 최대 3회**, 잔여 결함이 줄지 않으면(진전 없음) 3회 전이라도 보류 전환. 소진·진전없음 시 보류 보고로 ai-pm 릴레이 → 담당자 판단(directing 회귀/범위 축소/수동 개입) |
...
- **입력 최소화·증분 회귀·반환 규약** — doer 호출은 [`stages/spec.md`](../strategies/stages/spec.md) §입력 범위 규약의 필수 입력만 넘기고, 회귀는 지적된 절만 고치게 하며, doer 는 결론 요약·산출물 경로·후속 항목만 회신한다(사양 본문 반환 금지 — [`agents.md`](../strategies/agents.md) §실행 규칙).
