...
- **관련 Redmine 일감**: 작업세션 이슈 번호(업무 트리의 루트)와 **그룹 일감** 번호 — `기능` 일감의 부모 좌표이므로 반드시 전달받는다([`work-tracking.md`](../strategies/work-tracking.md) §계층·연관). 그 외 기존 `기능` 일감·참조 `사양` 일감(있으면).
...
- **미러**: 각 Phase 1건 → `기능` 일감 1건([`work-tracking.md`](../strategies/work-tracking.md) §단계별 미러링). 진척(작성→리뷰→검증→완료)에 따라 상태 동기화.
- **계층·연관**: `기능` 일감은 **그룹 일감의 하위**로 등록하고(`parent_issue_id` 필수), 참조한 사양정의서의 `사양` 일감은 **연관 일감**으로 단다 — 계층은 업무 소속, 연관은 근거다([`work-tracking.md`](../strategies/work-tracking.md) §계층·연관). 등록 후 `parent` 를 **실측 확인**한다(누락 시 오류 없이 루트 일감이 된다 — [`work-tracking-redmine.md`](../strategies/work-tracking-redmine.md) §도구 함정).
