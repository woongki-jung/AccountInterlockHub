...
| `<COMPLETION_REDIRECT_URL>` | `https://www.naver.com` (**잠정** — 개발용 더미) | 배포 전 (출처 확정과 함께) | **연동 완료** 결과에서 사용자를 되돌려 보낼 **복귀 주소**([`docs/prd/PRD.md`](docs/prd/PRD.md) §수행 범위 6). 허브가 요청 처리 응답에 이 값을 그대로 실어 주고 결과 화면이 3초 뒤 이동한다(완료가 아닌 결과에는 싣지 않는다). ⚠️ 다른 잠정값과 달리 **주소의 출처 자체가 미확정**이라(수신처 응답 / 발송처 전달 값 / 상수 유지) 확정 시 값 교체만으로 끝나지 않고 응답 구성이 함께 바뀔 수 있다([`docs/prd/devspec/external-apis.md`](docs/prd/devspec/external-apis.md) §미결) |
